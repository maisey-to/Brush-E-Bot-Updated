...
bool DFRobotDFPlayerMini::begin(Stream &stream, bool isACK, bool doReset){
  ...
  if (isACK) {
    Serial.println("Trying to enable ACK");
    enableACK();
    Serial.println("After trying to enable ACK");
  }
  else{
    Serial.println("Trying to disable ACK");
    disableACK();
    Serial.println("After trying to disable ACK");
  }
  
  if (doReset) {
    Serial.println("Doing Reset");
    reset();
    Serial.println("Doing waitAvailable");
    waitAvailable(2000);
    delay(200);
    ...
    Serial.println("Not doing reset");
